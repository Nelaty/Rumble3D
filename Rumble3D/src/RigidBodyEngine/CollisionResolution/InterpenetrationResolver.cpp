													   ...
													   const Contact& contact) const
	{
		...
		for(unsigned i = 0; i < 2; i++)
		{
			...
			if(body)
			{
				...
				projection += glm::dot(-contact.getRelativeContactPosition(i), contact.getContactNormal()) * contact.getContactNormal();
				...
				else
				{
					...
					glm::vec3 targetAngularDirection = glm::cross(contact.getRelativeContactPosition(i), contact.getContactNormal());
